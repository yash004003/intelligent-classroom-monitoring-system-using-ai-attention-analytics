...
if q3 < 0.5
    ...
    fprintf(fid,'%s          %s          %s                  %s\r\n\n', str, date,s,'Present');
    fclose(fid);
    set(handles.text5,'string',['Hello ' str ' ,Your Entry has been Marked.'])
    run('MAIN_CODE.m');
    try
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if  exist('Entry_sheet.txt','file') == 2
    winopen('Entry_sheet.txt')
else
    msgbox('NO Entry SHEET TO DISPLAY','INFO...!!!','HELP','MODAL')
end

% --------------------------------------------------------------------
function DEL_Entry_Callback(hObject, eventdata, handles)
% hObject    handle to DEL_ATTENDENCE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if  exist('Entry_sheet.txt','file') == 2
   delete('Entry_sheet.txt')
   msgbox('Entry DELETED','INFO...!!!','MODAL')
else
    msgbox('NO Entry SHEET TO DELETE','INFO...!!!','HELP','MODAL')
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
x  = questdlg({'Resetting will Clear the followings: ';'1. Entry_sheet';'2. Database';'3. features.mat';'4. Info.mat';'Do u want to continue?'},'Please select...!!');
if strcmpi(x,'yes') == 1
    delete('Entry_sheet.txt')
    delete('features.mat')
    delete('info.mat')
